...
- [[LLM Performance Evaluation Methods]]: Complete catalog of LLM evaluation methods from §5.2 of the LLM技术报告 — automated metrics (Perplexity, BLEU, ROUGE, F1), task-specific benchmark suites mapped to the capability each probes (MMLU, HellaSwag, TruthfulQA, GSM8K, HumanEval/MBPP, BIG-bench, ARC, DROP, SuperGLUE), leaderboards, human evaluation, LLM-as-a-Judge (G-Eval, RTS), multimodal & dialogue benchmarks, and the challenges of evaluation.
